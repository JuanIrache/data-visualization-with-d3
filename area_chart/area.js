const { select, scaleLinear, json, extent, max, axisLeft, axisBottom, area, curveBasis } = d3;
const svg = select('svg');
const width = +svg.attr('width');
const height = +svg.attr('height');

const render = data => {
  data = data[1].streams.ACCL.samples;
...
json('./hero6_1_ACCL.json').then(render);
